...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for elements in symbol_quantity_list:
        ...
        mass = elements[ATOMIC_MASS_INDEX]

        element_mass = periodic_table_dict[element]
        atomic_mass = element_mass[ATOMIC_MASS_INDEX]

        tot_mass += atomic_mass * mass
    return tot_mass
...
def main():
    ...
    mass = float(input('Enter the mass in grams of the formula: '))
    #call various functions and prepare to display
    periodic_table = make_periodic_table()
    broke_formula = parse_formula(formula, periodic_table)
    molar_mass = compute_molar_mass(broke_formula, periodic_table)
    moles = mass / molar_mass
    #print values
    print(round(molar_mass, 5))
    print(round(moles, 5))
    #see if user formula is a known compound
    known_mole = known_molecules()
    mole_name = get_formula_name(formula, known_mole)
    print(f'The formula you entered is {mole_name}')
