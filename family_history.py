...
def print_death_age(people_dict):
    ...
    """
    pass
    for person_key, item in people_dict.items():
        name = item[NAME_INDEX]
        death = item[DEATH_YEAR_INDEX]
        birth = item[BIRTH_YEAR_INDEX]
        age = death - birth
...
def count_genders(people_dict):
    ...
    print(f'{m_count} males')
    print(f'{f_count} females')
...
def print_marriages(marriages_dict, people_dict):
    ...
    for marriage_key, item in marriages_dict.items():
        husband = item[HUSBAND_KEY_INDEX]
        wife = item[WIFE_KEY_INDEX]
        wedding = item[WEDDING_YEAR_INDEX]
        ...
        print(f'{husband_name} {husband_age} > {wedding} < {wife_age} {wife_name}')
